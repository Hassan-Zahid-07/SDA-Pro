...
A professional React-based SOC Analyst Dashboard prototype is included in `soc-dashboard/`. It provides user-friendly frontend evidence for the MVC and Event-Driven requirements by visualizing:

- Real-time alert stream from Splunk SIEM, Palo Alto Firewall, CrowdStrike EDR, and threat intel feeds
- Incident queue with severity, assigned analyst, state, strategy, and risk score
- Incident lifecycle tracker for the State Pattern
- Response action panel for Block IP, Isolate Endpoint, Quarantine File, and Escalation actions
- Threat intelligence reputation panel showing Adapter + Proxy behavior
- Audit log showing Observer/EventBus-style updates

### Dashboard MVC Structure

| Layer | Evidence |
|---|---|
| Model | `soc-dashboard/src/models/` |
| View | `soc-dashboard/src/views/` |
| Controller | `soc-dashboard/src/controllers/` |
| Event Service | `soc-dashboard/src/services/mockEventBus.js` |
